...
export default async function AdminPage() {
  ...
  if (!ctx.profile?.is_admin) {
    return <div className="pt-10 text-center text-muted-foreground">🔒 Admins only.</div>;
  }
  ...
  return (
    <div className="space-y-8">
      <h1 className="pt-2 text-2xl font-extrabold">Admin</h1>
      ...
      <section className="space-y-4 rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <form action={setLock} className="flex flex-wrap items-end gap-2">
          <label className="text-sm">
            ...
            <input type="datetime-local" name="lock_at" defaultValue={toLocalInput(settings?.lock_at ?? null)} className="rounded border border-border bg-input/30 px-2 py-1" />
          </label>
          <button className="rounded-lg bg-neon px-3 py-1.5 text-sm font-semibold text-neon-foreground">Save lock time</button>
        </form>

        <div className="flex flex-wrap gap-3">
          <form action={freezeTiers}>
            <button disabled={!!settings?.tiers_frozen_at} className="rounded-lg border border-neon/50 bg-neon/10 px-3 py-1.5 text-sm font-bold text-neon disabled:opacity-50">
          ...
          <form action={setComplete}>
            ...
            <button className="rounded-lg border border-border px-3 py-1.5 text-sm font-semibold hover:bg-accent/40">
      ...
      <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <ul className="space-y-1">
          {(entries ?? []).map((e) => (
            <li key={e.id} className="flex items-center gap-2 border-b border-border py-1.5 text-sm last:border-0">
              <span className="flex-1">
                {e.display_name}
                {!e.submitted_at && <span className="ml-2 text-xs text-muted-foreground">(draft)</span>}
              </span>
              <span className={e.paid ? "text-neon" : "text-destructive"}>
                {e.paid ? "paid" : "unpaid"}
              </span>
              <form action={togglePaid}>
                <input type="hidden" name="entry_id" value={e.id} />
                <input type="hidden" name="paid" value={(!e.paid).toString()} />
                <button className="rounded border border-border px-2 py-0.5 text-xs hover:bg-accent/40">{e.paid ? "mark unpaid" : "mark paid"}</button>
              </form>
            </li>
          ))}
        </ul>
      ...
      <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <p className="mb-3 text-xs text-muted-foreground">Manual overrides stick — the feed won&apos;t overwrite them until you clear the override.</p>
        {(() => {
          const flagged = (matches ?? []).filter((m) => m.needs_attention);
          if (flagged.length === 0) return null;
          return (
            <div className="mb-3 rounded-lg border border-amber-400/40 bg-amber-400/10 p-3 text-sm text-amber-200">
              ...
              <ul className="mt-1 list-disc pl-5">
                {flagged.map((m) => {
                  ...
                  return (
                    <li key={m.fixture_id}>
                      {h?.name ?? "?"} vs {a?.name ?? "?"} <span className="text-amber-400/70">(#{m.fixture_id})</span>
                    </li>
                  );
                })}
              </ul>
        ...
        {(matches ?? []).length === 0 ? (
          <p className="text-sm text-muted-foreground">No matches yet. They appear after the first results sync.</p>
        ) : (
          <ul className="space-y-3">
            {(matches ?? []).map((m) => {
              ...
              return (
                <li key={m.fixture_id} className="rounded-lg border border-border p-2 text-sm">
                  ...
                  <form action={overrideResult} className="flex flex-wrap items-center gap-1">
                    ...
                    <input type="number" name="home_goals" defaultValue={m.home_goals ?? 0} min={0} className="w-14 rounded border border-border bg-input/30 px-1 py-0.5" />
                    <span>–</span>
                    <input type="number" name="away_goals" defaultValue={m.away_goals ?? 0} min={0} className="w-14 rounded border border-border bg-input/30 px-1 py-0.5" />
                    <select name="winner" className="rounded border border-border bg-input/30 px-1 py-0.5">
                      <option value="none">winner…</option>
                      {home && <option value={home.id}>{home.name}</option>}
                      {away && <option value={away.id}>{away.name}</option>}
                      <option value="draw">draw</option>
                    </select>
                    <button className="rounded bg-neon px-2 py-0.5 text-xs font-semibold text-neon-foreground">save</button>
                  </form>
                  {m.manual_override && (
                    <form action={clearOverride} className="mt-1">
                      <input type="hidden" name="fixture_id" value={m.fixture_id} />
                      <button className="text-xs text-muted-foreground underline">clear override</button>
                    </form>
                  )}
                </li>
              );
      ...
      <Link href="/" className="block text-center text-sm font-semibold text-neon hover:underline">← Back to pool</Link>
    </div>
  );
}

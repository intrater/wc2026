...
function baseInput(overrides: Partial<FinishRaceInput> = {}): FinishRaceInput {
  ...
  return {
    entries: [
      { entryId: "miller", name: "Miller Intrater", total: 123, moneyShare: 0.30, winShare: 0.03 },
      { entryId: "michael", name: "Michael Cranford", total: 122, moneyShare: 0.55, winShare: 0.36 },
      { entryId: "charlie", name: "Charlie French", total: 122, moneyShare: 0.45, winShare: 0.17 },
      { entryId: "zach", name: "Zach Fishman", total: 113, moneyShare: 0.40, winShare: 0.24 },
      { entryId: "cooper", name: "Cooper Benson", total: 87.5, moneyShare: 0.0, winShare: 0.0 },
    ],
    ...
    teamMap,
    groupWinner: "Charlie French",
    groupRunnerUp: "Michael Cranford",
    ...overrides,
  };
}
...
describe("buildFinishRace", () => {
  it("orders contenders by money odds, not points", () => {
    ...
    expect(r.contenders[0].name).toBe("Michael Cranford"); // 55% money, though 2nd on points
    expect(r.contenders[0].rank).toBe(2); // rank badge reflects the points board
    expect(r.contenders.map((c) => c.name)).not.toContain("Cooper Benson"); // 0% drops off
  });

  it("surfaces each contender's still-alive teams, strongest tier first", () => {
    const r = buildFinishRace(baseInput());
    const michael = r.contenders.find((c) => c.name === "Michael Cranford")!;
  ...
  it("flags contenders who already banked a group-stage prize", () => {
    const r = buildFinishRace(baseInput());
    expect(r.contenders.find((c) => c.name === "Charlie French")!.bankedGroupPrize).toBe(true);
    expect(r.contenders.find((c) => c.name === "Miller Intrater")!.bankedGroupPrize).toBe(false);
  });
  ...
  it("who-to-watch calls out a lurker when the leader is also the favorite", () => {
    ...
    input.entries = [
      { entryId: "miller", name: "Miller Intrater", total: 123, moneyShare: 0.5, winShare: 0.3 },
      { entryId: "michael", name: "Michael Cranford", total: 122, moneyShare: 0.30, winShare: 0.2 },
      { entryId: "charlie", name: "Charlie French", total: 121, moneyShare: 0.25, winShare: 0.1 },
      { entryId: "aa", name: "AA", total: 120, moneyShare: 0.2, winShare: 0.05 },
      { entryId: "zach", name: "Zach Fishman", total: 100, moneyShare: 0.22, winShare: 0.1 },
    ];
    const r = buildFinishRace(input);
